...
function drawHistChart(pts, period, startStr) {
  ...
  var n = pts.length;
  var cols = ['#5CC244', '#4ea8de', '#f05545', '#f0a030'];
  var keys = ['solar', 'batIn', 'gridIn', 'consumption'];
  var useLines = period.toLowerCase() === 'day';

  if (useLines) {
    // Line chart for daily (96 data points)
    for (var k = 0; k < 4; k++) {
      ctx.beginPath();
    ...
    for (var i = 0; i < n; i++) {
      if (i % Math.ceil(n / 12) === 0) {
        ...
        ctx.fillText(histLabel(i, period, startStr), x, H - p.b + 14);
      }
    }
  } else {
    // Bar chart for week/month/year
    var bw = cW / n, gw = bw * 0.8, sw = gw / 4;
    for (var i = 0; i < n; i++) {
